      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-28">
        ...
        <motion.section initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="relative pt-20 pb-20 text-center">
          ...
          <h1 className="text-5xl md:text-7xl xl:text-8xl font-black tracking-tight leading-[0.95] mb-8">
            ...
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-blue-600 to-purple-600 dark:from-indigo-300 dark:via-blue-300 dark:to-purple-300">Inspiration Hub</span>
          </h1>
          ...
          <div className="flex flex-wrap justify-center gap-5 mt-12">
            <button onClick={() => document.getElementById("discover-section")?.scrollIntoView({ behavior: "smooth" })} className="group px-8 py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-bold shadow-2xl shadow-indigo-500/25 hover:scale-[1.03] transition-all duration-300">
              <span className="flex items-center gap-3">Explore Inspirations <i className="fas fa-arrow-right text-sm group-hover:translate-x-1 transition-transform" /></span>
            </button>
            <button onClick={() => navigate("/stories")} className="px-8 py-4 rounded-2xl bg-white/80 dark:bg-white/[0.05] border border-white/60 dark:border-white/10 backdrop-blur-xl font-semibold hover:border-indigo-400/40 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300">
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] border border-white/60 dark:border-white/10 bg-white/80 dark:bg-white/[0.04] backdrop-blur-2xl shadow-[0_25px_80px_rgba(0,0,0,0.06)] p-6 md:p-10">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 mb-10">
              <div>
                ...
                <p className="mt-3 text-slate-600 dark:text-slate-400 max-w-2xl">Search through genres, themes, and iconic storytelling structures to unlock your next idea.</p>
              </div>
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-10">
              {genres.map((genre) => {
                ...
                return (
                  <button key={genre} onClick={() => setSelectedGenre(genre)} className={`relative overflow-hidden px-6 py-3.5 rounded-2xl text-sm font-semibold transition-all duration-300 border backdrop-blur-xl ${active ? "bg-gradient-to-r from-indigo-600 to-blue-600 text-white border-transparent shadow-xl shadow-indigo-500/25 scale-105" : "bg-white dark:bg-white/[0.04] border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-indigo-400/40 hover:text-indigo-600 dark:hover:text-indigo-300"}`}>
                    {genre}
                  </button>
                );
              })}
        ...
        {filteredStories.length > 0 ? (
          <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredStories.map((story) => (<StoryInspirationCard key={story.id} story={story} />))}
          </motion.div>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto rounded-[2.5rem] border border-white/60 dark:border-white/10 bg-white/80 dark:bg-white/[0.04] backdrop-blur-2xl p-16 text-center shadow-2xl">
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-400 leading-relaxed max-w-md mx-auto">Try adjusting your search keywords or explore different genres to uncover more creative ideas.</p>
            <button onClick={() => { setSearchQuery(""); setSelectedGenre("All"); }} className="mt-10 px-8 py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-bold hover:scale-[1.03] transition-all duration-300 shadow-xl shadow-indigo-500/20">
              Reset Filters
            </button>
          </motion.div>
        )}
        ...
        {/* Bottom CTA */}
        <motion.section initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mt-28">
          <div className="relative overflow-hidden rounded-[3rem] border border-white/60 dark:border-white/10 bg-gradient-to-br from-indigo-600 via-blue-600 to-purple-700 p-10 md:p-16 text-white shadow-[0_25px_100px_rgba(79,70,229,0.35)]">
            ...
            <div className="relative z-10 max-w-3xl">
              ...
              <p className="mt-6 text-lg text-white/80 leading-relaxed">Use StorySparkAI to transform inspirations into complete stories, plots, and cinematic adventures powered by intelligent AI storytelling.</p>
              <div className="flex flex-wrap gap-5 mt-10">
                <button onClick={() => navigate("/stories")} className="group px-8 py-4 rounded-2xl bg-white text-slate-900 font-bold hover:scale-[1.03] transition-all duration-300 shadow-2xl">
                  <span className="flex items-center gap-3">Start Writing <i className="fas fa-arrow-right group-hover:translate-x-1 transition-transform" /></span>
                </button>
                <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="px-8 py-4 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-xl font-semibold hover:bg-white/15 transition-all duration-300">
                  Explore More
                </button>
              </div>
            </div>
